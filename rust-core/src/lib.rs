...
mod tests {
    ...
    #[test]
    fn test_pulse_reader() {
        let mut pulse_reader = get_pulse_reader().unwrap();
        let apertures = pulse_reader.index.apertures.clone();
        for ap in apertures {
            let (records, ap_header) = pulse_reader.get_all_records(ap).unwrap();
            assert!(ap_header.num_pulses as usize == records.len());

            let (pulses, _ap_header) = pulse_reader.get_pulses(ap, None).unwrap();
    ...
    fn check_pulse_filter(
        ...
        let apertures = pulse_reader.index.apertures.clone();
        for ap in apertures {
            let (pulses, _ap_header) = pulse_reader.get_pulses(ap, Some(pulse_filter)).unwrap();
            for pulse in pulses {
    ...
    #[test]
    fn test_copy_apertures_to_new_file() {
        let mut pulse_reader = get_pulse_reader().unwrap();
        ...
        pulse_reader
            .copy_apertures_to_new_file(&apertures, &new_file_path)
            .unwrap();

        let mut new_pulse_reader = PulseReader::open(new_file_path).unwrap();
        assert_eq!(new_pulse_reader.index.apertures.len(), apertures.len());
        for ap in apertures {
            let (orig_records, _ap_header) = pulse_reader.get_all_records(ap).unwrap();
            let (new_records, _new_ap_header) = new_pulse_reader.get_all_records(ap).unwrap();
